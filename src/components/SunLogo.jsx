    ...
    <div style={{ display: "flex", alignItems: "center", gap: size > 40 ? 14 : 10 }}>
      <svg width={size} height={size} viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {rayAngles.map((angleDeg, index) => {
          ...
          return (
            <line key={index} x1={32 + innerRadius * Math.cos(rad)} y1={26 + innerRadius * Math.sin(rad)} x2={32 + outerRadius * Math.cos(rad)} y2={26 + outerRadius * Math.sin(rad)} stroke={gold} strokeWidth={isCardinal ? "1.8" : "1"} strokeLinecap="round" />
          );
        })}
      ...
      <div>
        <div style={{ fontFamily: "'Cormorant Garamond', 'Playfair Display', serif", fontSize: size > 40 ? "1.35rem" : "1rem", fontWeight: 600, color: textColor, letterSpacing: "0.12em", lineHeight: 1.15 }}>
          SUN OCEAN
        </div>
        <div style={{ fontFamily: "'Raleway', 'Outfit', sans-serif", fontSize: size > 40 ? "0.52rem" : "0.42rem", fontWeight: 500, color: gold, letterSpacing: "0.4em", marginTop: 1 }}>
          R E A L T Y
        </div>
      </div>
    </div>
