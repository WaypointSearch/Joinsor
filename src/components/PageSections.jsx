    ...
    <section style={{ background: COLORS.sand, padding: "80px 24px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <FadeIn><div style={{ textAlign: "center", marginBottom: 64 }}>
          <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: "0.75rem", letterSpacing: "0.3em", color: COLORS.gold, textTransform: "uppercase", marginBottom: 12, fontWeight: 500 }}>Why Agents Choose Us</div>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(1.8rem, 4vw, 2.6rem)", color: COLORS.navy, margin: 0, fontWeight: 700 }}>A Brokerage That Works For You</h2>
        </div></FadeIn>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 32 }}>
          {items.map((item, i) => (
            <FadeIn key={i} delay={i * 0.12}>
              <div style={{ background: COLORS.white, padding: 36, borderRadius: 2, borderTop: `2px solid ${COLORS.gold}`, transition: "transform 0.3s, box-shadow 0.3s", cursor: "default", minHeight: 200 }}
    ...
    <section id="commission" style={{ background: COLORS.navy, padding: "100px 24px", position: "relative" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <FadeIn><div style={{ textAlign: "center", marginBottom: 56 }}>
          ...
          <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "1rem", color: "rgba(255,255,255,0.55)", maxWidth: 520, margin: "0 auto", fontWeight: 300 }}>Every agent gets the same plan. Work your own deals, earn $250 per showing on our rental leads, or do both.</p>
        </div></FadeIn>
        <FadeIn delay={0.1}><div style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 3, overflow: "hidden" }}>
          {rows.map((row, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 20, padding: "24px 32px", borderBottom: i < 3 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
        ...
        <FadeIn delay={0.25}><div style={{ display: "flex", justifyContent: "center", gap: 24, marginTop: 40, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <button onClick={onApplyClick} style={{ background: COLORS.gold, color: COLORS.navy, border: "none", padding: "14px 32px", fontFamily: "'Outfit', sans-serif", fontSize: "0.8rem", letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 600, cursor: "pointer", borderRadius: 2 }}>Join Now</button>
        </div></FadeIn>
      </div>
    </section>
  );
...
export function HowItWorks({ onApplyClick }) {
  const steps = [
    { n: "01", t: "Apply Online", d: "Complete your application, review and e-sign your documents, upload your license, and pay $99. All done in minutes." },
    { n: "02", t: "Get Set Up", d: "Receive your welcome email with CRM login. Set your preferred areas and showing availability." },
    { n: "03", t: "Start Earning", d: "Take rental showing leads for $250 each, close your own sales at full commission, bring in your own tenants at 90% — mix and match however you like." },
    { n: "04", t: "Grow With Us", d: "Top showing agents can advance to Rental Coordinator or full pipeline roles. We invest in agents who produce." },
  ];
  return (
    <section id="how-it-works" style={{ background: COLORS.sand, padding: "100px 24px" }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 0 }}>
          {steps.map((s, i) => (
            <FadeIn key={i} delay={i * 0.12}><div style={{ padding: "36px 28px", borderLeft: i ? `1px solid ${COLORS.warmSand}` : "none" }}>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: "2.2rem", color: COLORS.gold, opacity: 0.35, fontWeight: 700, marginBottom: 12 }}>{s.n}</div>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.1rem", color: COLORS.navy, margin: "0 0 10px", fontWeight: 600 }}>{s.t}</h3>
              <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "0.88rem", color: COLORS.slate, lineHeight: 1.6, margin: 0, fontWeight: 300 }}>{s.d}</p>
            </div></FadeIn>
          ))}
        </div>
        <FadeIn delay={0.5}><div style={{ marginTop: 56, background: COLORS.white, padding: 40, borderRadius: 2, borderLeft: `3px solid ${COLORS.gold}`, display: "flex", gap: 32, flexWrap: "wrap", alignItems: "center" }}>
          <div style={{ flex: 1, minWidth: 250 }}>
            ...
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "0.9rem", color: COLORS.slate, lineHeight: 1.7, margin: 0, fontWeight: 300 }}>Newly licensed agents looking for showing income, part-time agents wanting flexibility, or experienced producers who want to keep 100% of their sales commission. One plan fits all.</p>
          </div>
          <button onClick={onApplyClick} style={{ background: COLORS.navy, color: COLORS.white, border: "none", padding: "14px 32px", fontFamily: "'Outfit', sans-serif", fontSize: "0.8rem", letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 500, cursor: "pointer", borderRadius: 2, whiteSpace: "nowrap" }}>Get Started</button>
        </div></FadeIn>
      </div>
    </section>
  );
    ...
    <section id="areas" style={{ background: `linear-gradient(135deg, ${COLORS.deepBlue} 0%, ${COLORS.navy} 100%)`, padding: "100px 24px" }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
          {areas.map((a, i) => (
            <FadeIn key={i} delay={i * 0.1}><div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)", padding: "32px 28px", borderRadius: 2, transition: "all 0.3s" }}
              ...
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.25rem", color: COLORS.white, margin: "0 0 10px", fontWeight: 600 }}>{a.name}</h3>
              <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "0.8rem", color: "rgba(255,255,255,0.4)", margin: 0, lineHeight: 1.6, fontWeight: 300 }}>{a.cities}</p>
            </div></FadeIn>
          ))}
        </div>
      </div>
...
export function FAQ() {
  ...
  const faqs = [
    { q: "Do I need a real estate license?", a: "Yes. You must hold an active Florida real estate sales associate or broker license to join Sun Ocean Realty." },
    { q: "What does $99/year cover?", a: "Your annual fee covers brokerage affiliation, online CRM and office tools, lead distribution, and full broker support. No desk fees, tech fees, or monthly charges." },
    { q: "How do the rental showings work?", a: "When a rental lead comes in, you'll receive a showing assignment to your phone. Meet the prospect, show them 2-3 properties. If they sign a lease and move in, you earn $250. We handle all the offers and paperwork." },
    { q: "Can I work my own deals AND take showing leads?", a: "Absolutely. Every agent has the full commission structure — 100% on your sales, 90% on your rentals, and $250 per showing on our leads. Most agents do both." },
    { q: "What areas can I work in?", a: "Broward, Miami-Dade, Palm Beach, Martin, and St. Lucie counties. Choose your preferred areas and change them anytime." },
    { q: "Is everything really remote?", a: "Yes. We are fully remote. All paperwork, CRM access, lead management, and communication happens online." },
    { q: "How do I sign up?", a: "Click 'Join Now,' complete your application, review and e-sign your documents, upload your license, and pay $99. Takes just a few minutes." },
    { q: "Do I need to join a REALTOR® association?", a: "Yes — Miami Association of REALTORS®, Fort Lauderdale/Palm Beaches, St. Lucie, Orlando, or Greater Tampa. If you choose not to join, notify the broker to be transferred to our non-affiliated entity." },
  ];
  return (
    <section id="faq" style={{ background: COLORS.sand, padding: "100px 24px" }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {faqs.map((f, i) => (
            <FadeIn key={i} delay={i * 0.05}><div style={{ background: COLORS.white, borderRadius: 2, overflow: "hidden" }}>
              <button onClick={() => setOpenIndex(openIndex === i ? null : i)} style={{ width: "100%", textAlign: "left", background: "none", border: "none", padding: "20px 24px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
                <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: "0.95rem", color: COLORS.navy, fontWeight: 500 }}>{f.q}</span>
                <span style={{ color: COLORS.gold, fontSize: "1.2rem", transition: "transform 0.3s", transform: openIndex === i ? "rotate(45deg)" : "rotate(0deg)", flexShrink: 0 }}>+</span>
              </button>
              <div style={{ maxHeight: openIndex === i ? 300 : 0, overflow: "hidden", transition: "max-height 0.4s ease" }}>
                <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "0.9rem", color: COLORS.slate, lineHeight: 1.7, margin: 0, padding: "0 24px 20px", fontWeight: 300 }}>{f.a}</p>
              </div>
            </div></FadeIn>
          ))}
        </div>
    ...
    <section style={{ background: `linear-gradient(135deg, ${COLORS.navy} 0%, ${COLORS.deepBlue} 100%)`, padding: "80px 24px", textAlign: "center" }}>
      <div style={{ maxWidth: 500, margin: "0 auto", display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "0.85rem", color: "rgba(255,255,255,0.4)", marginTop: 24, fontWeight: 300 }}>
          Questions? Call or text <a href={`tel:${BROKER_PHONE}`} style={{ color: COLORS.gold, textDecoration: "none" }}>{BROKER_PHONE}</a>
        </p>
      </div>
    </section>
  );
    ...
    <footer style={{ background: "#060e1a", padding: "40px 24px 24px", borderTop: "1px solid rgba(201,168,76,0.1)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16, marginBottom: 24 }}>
          {/* Logo only — no separate "Sun Ocean Realty" text */}
          <SunLogo size={30} light />
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 24, flexWrap: "wrap", padding: "20px 0", borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <svg width="28" height="28" viewBox="0 0 32 32" fill="none"><rect x="3" y="14" width="26" height="15" stroke="rgba(255,255,255,0.45)" strokeWidth="1.2" fill="none" /><path d="M16 3 L3 14 L29 14 Z" stroke="rgba(255,255,255,0.45)" strokeWidth="1.2" fill="none" /><text x="16" y="24.5" textAnchor="middle" fill="rgba(255,255,255,0.45)" fontSize="6" fontFamily="sans-serif" fontWeight="700">=</text></svg>
